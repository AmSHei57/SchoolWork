...
public class UserProcess {
	...
	CoffSection cSection = coff.getSection(vaddr);
	int vpn = cSection.getFirstVPN()+offset;

	TranslationEntry tEntry = pageTable[vpn];
	tEntry.readOnly = cSection.isReadOnly();
	int ppn = tEntry.ppn;
	...
	for (int s=0; s<coff.getNumSections(); s++) {
	    CoffSection cSection = coff.getSection(s);
	    if (cSection.getFirstVPN() != numPages) {
		coff.close();
		Lib.debug(dbgProcess, "\tfragmented executable");
		return false;
	    }
	    numPages += cSection.getLength();
	}
	...
	// program counter initially points at the program tEntry point
	...
	for (int s=0; s<coff.getNumSections(); s++) {
	    ...
	    Lib.debug(dbgProcess, "\tinitializing " + cSection.getName()
		      + " cSection (" + cSection.getLength() + " pages)");

	    for (int i=0; i<cSection.getLength(); i++) {
		int vpn = cSection.getFirstVPN()+i;
    ...
    private int handleCreate(int name) {
		...
		String hCreate = readVirtualMemoryString(name,n.length());
    	
    	OpenFile executable = ThreadedKernel.fileSystem.open(hCreate, true);
    	list.of.add(executable);
    	return list.of.indexOf(executable);
    }
    ...
    private int handleOpen(int name) {
    	...
    	OpenFile executable = ThreadedKernel.fileSystem.open(hCreate, false);
    	list.of.add(executable);
    	return list.of.indexOf(executable);
    }


    private void handleRead(int fileDescriptor, int buffer, int count) {
    	OpenFile sOpenFile = list.of.get(list.of.indexOf(fileDescriptor));
    	ArrayList<byte> bytes = new ArrayList<byte>();
    	byte[] bytes_temp = new byte[count+1];
    	sOpenFile.read(bytes_temp, 0, count);
    	
    	writeVirtualMemory(0,bytes_temp);
    }

    private void handleWrite(int fileDescriptor, int buffer, int count) {
		OpenFile sOpenFile = list.of.get(list.of.indexOf(fileDescriptor));
		byte[] bytes_temp = new byte[count + 1];
		int bytes_read = readVirtualMemory(0, bytes_temp);
		sOpenFile.write(bytes_temp, 0, count);
	}

	private void handleClose(int fileDescriptor) {
		OpenFile sOpenFile = list.of.get(list.of.indexOf(fileDescriptor));
		sOpenFile.close();
	}

	private void handleUnlink(int fileName) {
    	String JCSMC = Integer.toString(fileName);
		String NameOfFile = readVirtualMemoryString(fileName, JCSMC.length());
	...
	private void handleExec(int fileDescriptor, int argc, int argv) {
		//execute
		if (argc < 1) {
	...
	private void handleJoin(int pid, int status) {
		...
		if (childFlag == false) {
			Lib.debug(dbgProcess,
					"[UserProcess.handleJoin] "
							...
							+ " doesn't have a child with pid=" + pid);
			return -1;
		}
